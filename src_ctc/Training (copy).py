...
input_samples_op, input_labels_op = util_training.input_pipeline(TRAIN_FILENAMES)

# Pass True in when it is in the trainging mode
mode = tf.placeholder(tf.bool, name='mode')

input_labels_dense = tf.sparse_tensor_to_dense(input_labels_op)
...
with tf.name_scope('ctc_loss'):
    ...
    loss = tf.nn.ctc_loss(input_labels_op, logits, sequence_length=FLAGS.batch_size, time_major=False)

# Accuracy calculations
with tf.name_scope('accuracy'):
    predictions = tf.argmax(logits, 1, name='predictions')
    predictions = tf.Print(predictions,[predictions])
    correct_predictions = tf.nn.in_top_k(logits, input_labels_dense, 1)
...
# Group summaries.
summaries_training = tf.summary.merge([summary_trian_loss, summary_train_acc, summary_learning_rate, summary_images])
    ...
    for epoch in range(1, FLAGS.num_epochs + 1):
        ...
        for i in range(FLAGS.epoch_length):
            ...
            if (step % FLAGS.checkpoint_every_step) == 0:
                ...
                train_summary, correct_predictions_training, loss_training, _ = sess.run(request_output, feed_dict=feed_dict)
