    ...
    with tf.name_scope("network"):
        ...
        for batch_itr in range(BATCH_SIZE):
            ...
            with tf.name_scope("flatten"):
                pool5_flat = tf.reshape(pool5, [-1, FRAMES_PER_CLIP * IMAGE_SIZE[0] * IMAGE_SIZE[1] * 512])
            print('pool5+flat')
            ...
            batch_features.append(dropout2)

        batch_out = tf.reshape(batch_features, [BATCH_SIZE, CLIPS_PER_VIDEO, 4096])
        # Add dropout operation
        with tf.name_scope("LSTM"):
            lstm_cell = tf.contrib.rnn.BasicLSTMCell(num_units=512)
            lstm_outputs, _ = tf.nn.dynamic_rnn(lstm_cell, batch_out, dtype=tf.float32)
