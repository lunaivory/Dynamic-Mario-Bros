...
def video_preprocessing_training_op(video_op):
    with tf.name_scope('Single_gesture_video_preprocessing_training'):
        ...
        clip_op = tf.reshape(video_op, [constants_3dcnn.CLIPS_PER_VIDEO * constants_3dcnn.FRAMES_PER_CLIP_PP] + list(IMAGE_SIZE))
...
def read_and_decode(filename_queue):
    ...
    with tf.name_scope('TFRecordDecoding'):
        ...
        seq_label_dense = tf.reshape(seq_label_dense, [constants_3dcnn.FRAMES_PER_CLIP_PP])
        seq_label_dense = tf.slice(seq_label_dense, size=[constants_3dcnn.FRAMES_PER_CLIP], begin=jittering)

        # get per clip labels to train 3dcnn
        one_hots = tf.stack([tf.one_hot(indices=i,depth=21) for i in tf.unstack(seq_label_dense)])
        counter = tf.reduce_sum(one_hots, axis=0)
        seq_label_clip = tf.argmax(counter, axis=0)
        #seq_label_clip = tf.decode_raw(features_encoded['clip_label'], tf.int32)
        seq_label_clip = tf.reshape(seq_label_clip, [constants_3dcnn.CLIPS_PER_VIDEO])
        seq_label_clip = tf.cast(seq_label_clip, dtype=tf.int32)
...
def input_pipeline(filenames):
    with tf.name_scope('input_pipeline_training_3dcnn'):
        ...
        batch_rgb, batch_label, batch_dense_label, batch_clip_label = tf.train.shuffle_batch([data, label, dense_label, clip_label],
                                                        batch_size=constants_3dcnn.BATCH_SIZE,
                                                        capacity=constants_3dcnn.QUEUE_CAPACITY,
                                                        min_after_dequeue=int(constants_3dcnn.QUEUE_CAPACITY / 2),
                                                       )
        ...
        batch_dense_label = tf.reshape(batch_dense_label, [constants_3dcnn.BATCH_SIZE*constants_3dcnn.FRAMES_PER_VIDEO])

        # reshape clip labels (frame by frame annotations)
        batch_clip_label = tf.reshape(batch_clip_label, [constants_3dcnn.BATCH_SIZE*constants_3dcnn.CLIPS_PER_VIDEO])
