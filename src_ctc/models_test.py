...
def dynamic_mario_bros(input_layer, dropout_rate, mode):
    ...
    with tf.name_scope("network"):
        ...
        with tf.name_scope("cnn1"):
            input_conv1 = tf.squeeze(input_layer)
        ...
        with tf.name_scope("pooling6"):
            pool6 = tf.layers.max_pooling3d(inputs=pool5, pool_size=[1, 2, 2], strides=[1, 2, 2], padding="same")

        with tf.name_scope("pooling7"):
            pool7 = tf.layers.max_pooling3d(inputs=pool6, pool_size=[1, 2, 2], strides=[1, 2, 2], padding="same")

        # Flatten tensor into a batch of vectors
        # shape=[batch_size, max_time, flat
        with tf.name_scope("flatten"):
            pool5_flat = tf.reshape(pool7, shape=[-1, 3 * 3 * 128])
        ...
        # Flatten tensor into correct format for LSTM, here batch size is 1, max time is 250
        with tf.name_scope("flatten_LSTM"):
            dropout2_flat = tf.reshape(pool5_flat, shape=[1, 250, 4096])
        #
        # # Add dropout operation
        # with tf.name_scope("LSTM"):
        #     lstm_cell = tf.contrib.rnn.BasicLSTMCell(num_units=512)
        #     lstm_outputs, _ = tf.nn.dynamic_rnn(lstm_cell, dropout2_flat, dtype=tf.float32, time_major=False,
        #                                         sequence_length=[250])
        #     # TODO add dropout wrapper for LSTM

        # Logits layer
        with tf.name_scope("logits"):
            logits = tf.layers.dense(inputs=dropout2_flat, units=22)
